...
@WebServlet("/updatepagec")
public class UpdatePageClass extends HttpServlet {
		...
		if(conn!=null)
		{
			try {
				PreparedStatement stmt= 
						conn.prepareStatement("select * from classes where id=?");
				stmt.setInt(1, id);
				ResultSet rs= stmt.executeQuery();
				out.println("<h1>Welcome Admin, Update your data</h1>");
				if(rs.next())
				{
					out.print("<form action='updatec' method='post'>");
					out.print("<input type='hidden' name='id' value='"+rs.getString(1)+"'>");
					out.print("C Name: <input type='text' name='cname' value='"+rs.getString(2)+"'><br><br>");
					out.print("T Name: <input type='text' name='tname' value='"+rs.getString(3)+"'><br><br>");
